...
def scan_test_files(
    ...
    """Return test files matching criteria, respecting the same scoping filters."""
    exclude_paths = exclude_paths or []
    search_root = scope_path or project_root
    results: list[Path] = []
    for ext in extensions:
...
def _is_test_file(path: Path) -> bool:
    ...
    return (
        any(p in name for p in _TEST_PATTERNS)
        or path.name.endswith(_JAVA_TEST_SUFFIX)
